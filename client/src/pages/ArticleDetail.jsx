            ...
            <div className="space-y-4">
              <div className="relative aspect-[16/9] w-full overflow-hidden rounded-2xl bg-gray-950 shadow-md">
                ...
                {activeMedia?.type === 'video' && (
                  <div 
                    key={`video-${activeMedia.url}`}
                    className="w-full h-full text-left animate-in fade-in duration-300" 
                    dir="ltr"
                  >
                    <Plyr 
                      source={{
                ...
                <div className="flex flex-wrap gap-3 overflow-x-auto py-1 custom-scrollbar">
                  ...
                  {article.images?.map((img, idx) => {
                    ...
                    return (
                      <button
                        key={`img-btn-${idx}`}
                        onClick={() => handleMediaSelect({ type: 'image', url: imgUrl, caption: imgCaption })}
                  ...
                  {article.videos?.map((vidUrl, idx) => (
                    <button
                      key={`vid-btn-${idx}`}
                      onClick={() => handleMediaSelect({ type: 'video', url: vidUrl })}
                      className={`relative w-20 h-14 sm:w-24 sm:h-16 rounded-xl overflow-hidden bg-slate-900 border-2 transition-all shrink-0 flex items-center justify-center ${
                        activeMedia?.url === vidUrl ? 'border-[var(--color-secondary)] scale-95 shadow-sm' : 'border-transparent opacity-70 hover:opacity-100'
                      }`}
                    >
                  ...
                  {article.youtube_videos?.map((videoId, idx) => (
                    <button
                      key={`yt-btn-${idx}`}
                      onClick={() => handleMediaSelect({ type: 'youtube', id: videoId })}
                      className={`relative w-20 h-14 sm:w-24 sm:h-16 rounded-xl overflow-hidden bg-gray-100 border-2 transition-all shrink-0 ${
                        activeMedia?.id === videoId ? 'border-[var(--color-secondary)] scale-95 shadow-sm' : 'border-transparent opacity-70 hover:opacity-100'
                      }`}
                    >
                      <img 
